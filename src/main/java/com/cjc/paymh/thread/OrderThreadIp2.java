 ...
 * @author chenjc23273
 */
public class OrderThreadIp2 extends Thread {

    protected static final Logger logger = LoggerFactory.getLogger(OrderThreadIp2.class);
        ...
        try {
            ...
            if (DateUtil.isBuyTime(buyTime,beginTime,endTime)){
                ...
                if(!isQuery){
                    ...
                    if(!MainThreadIp2.endThreadLocal.get()){
                        orderid_to_epay = PayService.addOrderIp2(paramMap,cbgSafeCode,cookie,index);
                    }
                    if(StringUtils.isNotBlank(orderid_to_epay) && !MainThreadIp2.endThreadLocal.get()){
                        MainThreadIp2.endThreadLocal.set(true);
                        if("-1".equals(orderid_to_epay)){
                            ...
                            String epay_pay_url = PayService.getOrderInfoIp(orderid_to_epay,cbgSafeCode,cookie,t,1);
                            logger.info("增加订单里的===============获取支付地址完成 " + DateUtil.currentDateR() + " " + epay_pay_url);
                            if(StringUtils.isNotBlank(epay_pay_url)){
                                String payNo = PayService.getPayNoIp(epay_pay_url,payCookie,t,1);
                                logger.info("增加订单里的===============访问支付地址完成 " + payNo + " " + DateUtil.currentDateR());
                                if (StringUtils.isBlank(payNo)){
                                    payNo = PayService.getPayNoIp(epay_pay_url,payCookie,t,1);
                                }
                                if(StringUtils.isNotBlank(payNo)){
                                    String str2 = PayService.goPayFinalIp(payNo,(String)paramMap.get("confirm_price_total"),payPassword,payCookie,t,1);
                                }
                            }
//                            MainThreadIp2.endThreadLocal.set(false);
                        }
                    }
                }else{
...
//                    String orderid_to_epay = PayService.queryOrderIp(cbgSafeCode,cookie,httpClient);
                    ...
                    if(StringUtils.isNotBlank(orderid_to_epay)){
                        ...
                        if(StringUtils.isNotBlank(epay_pay_url) && !MainThreadIp2.endThreadLocal.get()){
                            MainThreadIp2.endThreadLocal.set(true);
